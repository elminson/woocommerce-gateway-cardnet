...
Version: 1.0.0
Author: Elminson De Oleo Baez (edeoleo@gmail.com)
...
include_once(dirname(__FILE__) . '/responseCodes.php');
...
function woocommerce_cardnet_init()
{
    ...
    class WC_Cardnet extends WC_Payment_Gateway
    {
		...
		public function __construct()
        {
            ...
            $this->currency = self::CURRENT_CURRENCY;
            $this->textactive = 0;
        ...
        public function init_form_fields()
        {
            $this->form_fields = array(
                ...
                'title' => array(
                    ...
                    'description' => __('Esto controla el titulo que el usuario visualizar	Cardnet Online Paymev                            á al momento de hacer checkout.', 'cardnet-woocommerce'),
                    'desc_tip' => true,
                ),
                'description' => array(
         ...
         * @return string
         **/
        protected function get_cardnet_args($order_id)
        {
            global $woocommerce;
            ...
            $cardnet_args = array(
                ...
                'Amount' => str_pad(str_replace('.', '', $order->order_total * 100), 12, '0', STR_PAD_LEFT),
        ...
        function generate_cardnet_form($order_id)
        {
            ...
            echo '<form action="' . $payment_address . '" method="POST" id="cardnet_payment_form" target="_top">
            ...
            switch ($codes[$state]) {
                ...
                case 'PENDING':
                    ...
                    if ($codes[$state] == 'APPROVED') {
                        ...
                        $order->update_status('on-hold', sprintf(__('Pago Pendiente: %s', 'cardnet-woocommerce'), CARNET_RESPONSE_CODE[$state]));
                ...
                case 'ERROR':
                    ...
                    $order->update_status('failed', sprintf(__('Pago rechazado por Cardnet. Error type: %s', 'cardnet-woocommerce'), (CARNET_RESPONSE_CODE[$state])));
                    $this->msg['message'] = $this->msg_declined;
                    $this->msg['class'] = 'woocommerce-error';
                    break;
                default:
                    $order->update_status('failed', sprintf(__('Pago rechazado por Cardnet. Error type: %s', 'cardnet-woocommerce'), (CARNET_RESPONSE_CODE[$state])));
                    $this->msg['message'] = $this->msg_cancel.' '.CARNET_RESPONSE_CODE[$state];
                    $this->msg['class'] = 'woocommerce-error';
                break;
            }
        ...
        function cardnet_confirmation_process($posted)
        {
            ...
            $state = $_POST['ResponseCode'];
			$posted['AuthorizationCode'] = '12345';
            if ('yes' == $this->debug) {
                $this->log->add('cardnet', 'Payment status: ' . CARNET_RESPONSE_CODE[$state]);
            }
